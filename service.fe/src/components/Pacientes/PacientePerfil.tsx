...
const renderDivertimento = (diversao: number | null | undefined) => {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-sm font-semibold text-tinta">
      ...
      <span className="text-xs text-aco">({diversao}/5)</span>
    </span>
  );
};
...
const renderAlertas = (teveProblemas: boolean | null | undefined) => {
  if (teveProblemas) {
    return (
      <span
        className="inline-flex items-center gap-1 rounded-full border border-capa/40 bg-capa/10 px-2.5 py-0.5 text-xs font-semibold text-capa-escura animate-pulse"
...
const PacientePerfil = () => {
  ...
  if (loading) {
    return (
      <div className="mx-auto w-full max-w-5xl px-4 py-10">
        <LoadingSpinner mensagem="A carregar perfil do paciente..." />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-10">
        ...
        <div className="mt-4 grid gap-4 grid-cols-2 sm:grid-cols-4">
          {RECOMPENSAS.map((rec) => {
            ...
            return (
              <div
                key={rec.id}
                className={`rounded-2xl border p-4 text-center transition ${desbloqueada ? "border-turbo/40 bg-turbo/10" : "border-tinta/15 bg-papel opacity-60"}`}
              >
                ...
                <span
                  className={`inline-block mt-3 rounded-full px-2 py-0.5 text-[9px] font-bold ${desbloqueada ? "bg-turbo/15 text-turbo-escuro" : "bg-tinta/10 text-aco"}`}
                >
      ...
      <section className="mt-6 rounded-3xl border border-tinta/15 bg-papel-claro p-6 shadow-sm">
        <h2 className="text-lg font-bold text-tinta">Sessões Recentes</h2>
        <p className="mt-1 text-sm text-aco">
          Últimas sessões de exercício realizadas pelo paciente (até 5 mais recentes).
        </p>
        {sessoesRecentes.length === 0 ? (
          <p className="mt-4 text-sm text-aco italic">Sem sessões registadas recentemente.</p>
        ) : (
          <div className="mt-4 overflow-x-auto rounded-2xl border border-tinta/15 bg-papel">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-tinta/15 bg-papel-claro">
                {sessoesRecentes.map(({ data, sessao }) => {
                  ...
                  return (
                    <tr key={sessao.idSessao} className="hover:bg-papel/60">
                      <td className="px-4 py-3.5 font-bold text-tinta">
                        {sessao.nomeExercicio}
                      </td>
                      <td className="px-4 py-3.5 text-aco whitespace-nowrap">
